...
import { db } from "@/db";
import { tournaments, extraMealOptions } from "@/db/schema";
...
export async function GET() {
  ...
  const options = await db.query.extraMealOptions.findMany({
    where: eq(extraMealOptions.tournamentId, tournament.id),
...
export async function PATCH(req: NextRequest) {
  ...
  const [updated] = await db
    ...
    .set(fields)
    .where(eq(extraMealOptions.id, id))
    .returning();
